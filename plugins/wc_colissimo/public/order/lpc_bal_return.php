...
class LpcBalReturn extends LpcComponent {
	...
	public function init() {
		add_action('woocommerce_order_details_before_order_table', array($this, 'addBalReturn'));
		...
		LpcHelper::enqueueStyle('lpc_bal_return', null, plugins_url('/css/lpc_bal_return.css', LPC_PUBLIC . 'init.php'));
	}
	...
	public function control(WP $wp) {
		...
		// Display
		die(LpcHelper::renderPartialInLayout(
			$partial,
			$data
		));
	}
	...
	protected function getListMailBoxPickingDatesResponse($payload) {
		try {
			...
			LpcLogger::debug(__METHOD__ . ' Error calling pickup', array($payload));
	...
	protected function sendPickUpConfirmation($payload, $returnTrackingNumber) {
		...
		try {
			...
			LpcLogger::debug(__METHOD__ . ' Error confirming pickup', array($payload));
	...
	protected function getReturnTrackingNumber(WC_Order $order) {
		...
		if (empty($inwardLabel)) {
			try {
				...
				LpcLogger::debug(__METHOD__ . ' Error generating return label on pickup confirmation', array('order' => $order->get_id()));
			}
		}
