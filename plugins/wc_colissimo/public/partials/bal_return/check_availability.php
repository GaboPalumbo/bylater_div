		...
		<form method="POST" action="<?= $args['urlBalReturn']; ?>">
			...
			if ($args['listMailBoxPickingDatesResponse']) {
				...
				<div>
					...
					<button class="lpc_balreturn_btn" type="submit"><?= esc_html__('Confirm pick-up', 'wc_colissimo'); ?></button>
				</div>
				<?php
			} else {
				?>
				<p class="lpc_balreturn_error"><b><?= esc_html__('This address is not eligible for MailBox pick-up.', 'wc_colissimo'); ?></b></p>
				<?php
			} ?>
