...
require_once LPC_FOLDER . DS . 'lib' . DS . 'tcpdf' . DS . 'tcpdf.php';
...
class LpcInvoiceGenerateAction extends LpcComponent {
	...
	public function generateInvoice($orderId, $filename, $destination) {
		try {
			...
			$pdf->writeHTML($this->printStoreInformationInHTML());
			$pdf->writeHTML($this->printBlanklines(2));
			$pdf->writeHTML($this->printOderTitle($order));
	...
	public function printOderTitle($order) {
		$output = '<h3>' . __(sprintf('[Order #%d]', $order->get_id()), 'wc_colissimo') . ' (' . wc_format_datetime($order->get_date_created()) . ')</h3>';
	...
	protected function printStoreInformationInHTML() {
		$store_address2 = '';
		if (get_option('woocommerce_store_address_2')) {
			$store_address2 = get_option('woocommerce_store_address_2') . '<br>';
		}
		$output_html = '';
		$output_html .= '<div><span style="font-weight: bold;font-size: 1.2em;">' . LpcHelper::get_option('lpc_company_name', '') . '</span><br>' . get_option('woocommerce_store_address') . '<br>' . $store_address2 . get_option('woocommerce_store_postcode') . ' <br>' . get_option('woocommerce_store_city') . ' <br>' . WC()->countries->countries[get_option('woocommerce_default_country')] . ' <br>
					     </div>';
